...
def FindID(content,area):
    ...
    if re.search('Netcool',content):
        ...
        if b:
            ...
            return 'Not Found!'
def FindKey(data):
    summaryList,sysList =data['summary'],data['sysname']
    messageID = []
    KeyDict={}
...
def dataPrepos(text):
    ...
    text=re.sub(r"\d{0,2}\s*\d{0,2}\.*\d{0,2}\.\d{0,2}", "",text)
...
def kmeans_blue_warning(in_df,keys):
    ...
    #ModelDict = {'messageid': messageID,'summary':Summary,'risk': Risk,'category':Category,'keys':keys}
    #out_df= pd.DataFrame(ModelDict, columns=['messageid', 'summary','risk','category','keys'])
    ModelDict = {'messageid': messageID, 'summary': Summary, 'risk': Risk, 'category': Category}
    out_df = pd.DataFrame(ModelDict, columns=['messageid', 'summary', 'risk', 'category'])
    return out_df
...
def main():
   ...
   # stopkey = [w.strip() for w in codecs.open('data/stopWord.txt', 'r',encoding='gb18030').readlines()]
    # tf-idf关键词抽取
    keys = getKeywords_tfidf(data, 1)
    result = kmeans_blue_warning(data,keys)
    result.to_csv("result/30days_result.csv",index=False)
